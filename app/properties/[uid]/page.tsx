...
export default async function LegacyPropertyRoute({ params }: { params: Promise<{ uid: string }> }) {
  const { uid } = await params;
  const lookup = await lookupPublicProperty(uid, { revalidate: SEO_REVALIDATE }).catch(() => null);
  ...
  if (lookup?.state === 'unavailable' && lookup.company.slug) permanentRedirect(`/${lookup.company.slug}/${uid}`);
  return <PropertyPage uid={uid} />;
}
