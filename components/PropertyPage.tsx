...
/** A property's public page. `uid` is the property's slug. */
export default function PropertyDetailPage({ uid }: { uid: string }) {
  ...
  // After mount, and a tick later so the route tracker has recorded this page.
  useEffect(() => {
    const t = setTimeout(() => setCanGoBack(hasInAppHistory()), 0);
  ...
  */
  ...
  if (notFound) return (
    <div className="cd-page">
      ...
      <div style={{ textAlign: 'center', padding: '80px 20px', color: '#5A5048' }}>
        ...
        <div style={{ marginTop: 12 }}><a href="/" style={{ color: BRAND, fontWeight: 700 }}>Browse all properties</a></div>
      </div>
      <Footer />
    </div>
  );
  ...
  const phoneDigits = property.agentPhone.replace(/\D/g, '');
  const canCall = property.allowContact && !!phoneDigits;
  const scrollToContact = () =>
    document.getElementById('contact-agent')?.scrollIntoView({ behavior: 'smooth', block: 'start' });

  return (
    <div className="cd-page cd-has-contact-bar">
      <Header
        brand={property.brand ?? toBrand(property.companyName, '', '', '')}
        companyHref={companyPath(property.companySlug, property.companyId)}
        title={property.name}
        subtitle={fullAddr}
      />
      <div className="cd-container" style={{ maxWidth: 1140, margin: '0 auto', padding: '26px 28px 70px' }}>
        {/*
          "Back" only when it stays on this site. Opened from a shared link (or
          refreshed) there is nothing of ours to go back to, so offer the
          company's other listings instead.
        */}
        {canGoBack ? (
          <button onClick={() => router.back()} style={backLinkStyle}><ArrowLeftIcon size={15} /> Back</button>
        ) : (
          <Link href={companyPath(property.companySlug, property.companyId)} style={backLinkStyle}>
            <ArrowLeftIcon size={15} /> {property.companyName ? `All ${property.companyName} listings` : 'All listings'}
          </Link>
        )}

        <div className="cd-detail-hero" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 6, borderRadius: 18, overflow: 'hidden', marginBottom: 22, height: 440 }}>
        ...
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
          ...
          {/* On phones this column falls below the description; the contact bar scrolls here. */}
          <div id="contact-agent" style={{ flex: '1 1 280px', minWidth: 270, display: 'flex', flexDirection: 'column', gap: 16, scrollMarginTop: 80 }}>
      ...
      {/*
        Phones: the agent card sits below the description, which can be long,
        so the ways to reach the agent stay pinned to the bottom of the screen.
      */}
      <div
        className="cd-contact-bar"
        style={{ position: 'fixed', left: 0, right: 0, bottom: 0, zIndex: 150, gap: 8, padding: '10px 12px calc(10px + env(safe-area-inset-bottom))', background: 'rgba(255,255,255,.96)', backdropFilter: 'blur(10px)', WebkitBackdropFilter: 'blur(10px)', borderTop: '1px solid #EAE6E0', boxShadow: '0 -6px 20px rgba(0,0,0,.06)' }}
      >
        {canCall ? (
          <>
            <a href={`tel:+${phoneDigits}`} onClick={() => track(EVENTS.AGENT_CONTACTED, { property_uid: uid, channel: 'call' })} style={{ flex: 1, minHeight: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, background: '#E8650A', color: '#fff', borderRadius: 12, fontSize: 14, fontWeight: 700 }}>
              <PhoneIcon size={16} /> Call
            </a>
            <a href={`https://wa.me/${phoneDigits}`} target="_blank" rel="noopener noreferrer" onClick={() => track(EVENTS.AGENT_CONTACTED, { property_uid: uid, channel: 'whatsapp' })} style={{ flex: 1, minHeight: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, background: '#fff', color: '#3B6D11', border: '1px solid #C0DD97', borderRadius: 12, fontSize: 14, fontWeight: 700 }}>
              <MessageCircleIcon size={16} /> WhatsApp
            </a>
          </>
        ) : null}
        <button type="button" onClick={scrollToContact} style={{ flex: 1, minHeight: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, background: canCall ? '#F5F2EE' : '#E8650A', color: canCall ? '#3A3530' : '#fff', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer' }}>
          <SendIcon size={16} /> {canCall ? 'Enquire' : 'Contact agent'}
        </button>
      </div>
